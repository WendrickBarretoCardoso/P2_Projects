...
// Lista fornecedores cadastrados
    ...
    try {
        await axios.get('http://localhost:3000/fornecedores').then(
            (result) => { result.data.forEach(({ _id, nome }) => console.log(_id + '- ' + nome)) }
        );
        console.log('------------------------------');
    } catch (error) {
        ...
        if (opcao !== -1) { // -1 -> Sair
            ...
            switch (opcao) {
                ...
                case 1: // Excluir
                    ...
                    if (excluir)
                        await axios.delete(`http://localhost:3000/produtos/${produto._id}`).then((result) => console.log(result.data.message));
                    break;
                case -1:
